...
public class BlogService {
    ...
    public List<BlogPublic> getAllOwnBlog() {
        //fix user_id = 1 (role: admin)
        List<BlogPublic> ownBlogsPage = blogRepository.findByUser_IdOrderByCreatedAtDesc(1);
        return ownBlogsPage;
    }
    ...
    //Thêm blog mới
    @Transactional
    public BlogPublic addNewBlog(UpsertBlogRequest request){
        //find category list
        List<Category> categories = categoryRepository.findByIdIn(request.getCategoryIds());

        //userid fixed to 1
        User user = userRepository.findById(1).orElseThrow(()-> {
            throw new NotFoundException("user id 1 not found");
        });
